...
const roles: { value: UserRole; label: string; desc: string }[] = [
  { value: 'buyer', label: 'Regular User', desc: 'Browse, save, and buy property. No dashboard.' },
...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    // Create user in-memory and log in
    const user = {
      id: `user-${Date.now()}`,
      email,
      fullName: name,
    ...
    toast.success('Account created successfully! Welcome to Vestra.');
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Start your property journey</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-4 py-2.5 text-sm text-gray-900 dark:text-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none"
              >
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          Already have an account? <Link to="/auth/login" className="text-emerald-600 font-medium hover:underline">Sign In</Link>
        </p>
      </div>
    </div>
  );
